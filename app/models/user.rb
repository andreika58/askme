...
class User < ApplicationRecord
  ...
  validates :username, format: { with: /\A[a-zA-Z0-9_]+\Z/, message: "Only latin characters, numbers and '_' symbols allowed" }
  validates :email, presence: true, 'valid_email_2/email':true
  validates :username, length: { maximum: 40, too_long: "Max allowed lenght is 40" }
  ...
  def encrypt_password
    if self.password.present?
      ...
      # Создаем хеш пароля - длинная уникальная строка, из которой невозможно
  ...
  def self.authenficate(email, password)
